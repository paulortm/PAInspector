...
public abstract class FieldCommand implements Command {
	...
	@Override
	abstract public void execute(Inspector insp, List<String> args) throws CommandException;
	
	/*
	 * Finds the field named fieldName on class clazz.
	 * If there are shadowed fields with that name the user is asked to chose one of them. 
	 */
	public Field findField(Inspector insp, Class<?> clazz, String fieldName) throws FieldNotFoundException, InvalidOptionException  {
		Map<String,Field> fields = ReflectionHelper.findAllFields(clazz, fieldName);
		Field field;
		if(fields.size() == 0) {
	...
	private Field askFieldFromUser(Inspector insp, Map<String,Field> fields) throws InvalidOptionException {
		...
		for(Map.Entry<String,Field>  entry: entrys) {
			f = entry.getValue();
			insp.println( entry.getKey() + ":\t" + Modifier.toString(f.getModifiers()) + " "
					+ f.getType() + " " + f.getName());
		}
